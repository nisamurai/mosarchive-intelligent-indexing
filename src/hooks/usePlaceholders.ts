...
export const usePlaceholders = () => {
  ...
  useEffect(() => {
    const fetchPlaceholders = async () => {
      try {
        ...
        const response = await fetch('/api/placeholders/all');
  ...
  const getMockStatistics = async (): Promise<MockStatistics | null> => {
    try {
      const response = await fetch('/api/placeholders/statistics/mock');
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
  ...
  const getMockDailyStatistics = async (): Promise<DailyStats[]> => {
    try {
      const response = await fetch('/api/placeholders/statistics/daily');
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
      return await response.json();
  ...
  const getReportSampleData = async (): Promise<ReportSampleData | null> => {
    try {
      const response = await fetch('/api/placeholders/report/sample');
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
      return await response.json();
