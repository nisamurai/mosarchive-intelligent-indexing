...
- `onUpload: (files: File[]) => void` - callback после загрузки
- `maxFiles?: number` - максимальное количество файлов (по умолчанию 10)
- `maxFileSize?: number` - максимальный размер файла (по умолчанию 50MB)
...
| Функция | Описание |
|---------|----------|
| **Увеличить** | Масштабирование изображения на 20% |
| **Уменьшить** | Масштабирование изображения на 20% |
| **Повернуть** | Поворот изображения на 90° |
| **Скачать** | Скачивание изображения документа |
| **Сброс масштаба** | Возврат к исходному масштабу |
